...
class JFormFieldJxtctag extends JFormField {

	protected	$_name = 'Jxtctag';

	protected function getInput()	{
		$db	= JFactory::getDBO();
		$q = "SELECT t.id, t.name FROM #__k2_tags as t WHERE t.published = '1' ORDER BY t.name";
		$db->setquery($q);
		$result=$db->loadObjectList();
		array_unshift($result,(object)array('id'=>0,'name'=>'ALL TAGS'));
		$size = count($result);
		//$size = ceil($size/10);
		if ($size < 5) $size = 5;
		if ($size > 20) $size = 20;
