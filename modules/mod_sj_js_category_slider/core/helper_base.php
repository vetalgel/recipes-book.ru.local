...
if (!class_exists('SJJSCategorySliderBaseHelper')) {
    /**
     * BaseHelper for com_content only.
     */
    abstract class SJJSCategorySliderBaseHelper
    {
        /**
            ...
            if (!isset(self::$image_cache[$hash][$item->product_id])) {
                        ...
                        switch ($type) {
                            case 'description':
                                ...
                                for ($i = 0; $i < count($inline_images); $i++) {
                                    if (file_exists($inline_images[$i]['src'])) {
                            ...
                            case 'image_product':
                                if ($type == 'image_product') {
                                    ...
                                    $image_path .= (isset($item->product_name_image) && $item->product_name_image != '') ? $item->product_name_image : $item->product_thumb_image;

                                    if (!empty($image_path) && file_exists($image_path) && is_file($image_path) == true) {
                                        $image = array(
                                            'src' => $image_path
                                        );
                                        $image['alt'] = $item->name;
                                ...
                                break;
                            case 'external':
                                $exf = $params->get($prefix . '_external_url', 'images/joomshopping/{product_id}/');
                ...
                if (count($images_path) == 0 && $params->get($prefix . '_placeholder', 1) == 1) {
                    ...
                    if (strpos($params->get($prefix . '_placeholder_path', null), 'nophoto') === false) {
                        $images_path[] = array('src' => $params->get($prefix . '_placeholder_path', null), 'class' => 'respl-placeholder');
                    } else {
                        $images_path[] = array('src' => $params->get($prefix . '_placeholder_path', null), 'class' => 'respl-nophoto');
                    }
                }
